...
SEED_ACTIVITIES: list[tuple[str, dict[str, object]]] = [
    (
        "organization_provisioning",
        {
            ...
            "purpose": "Local one-time provisioning of the single organization and its initial owner account via the operator-run provision_owner command. No public registration exists and no second organization can be created through the application.",
            "controller": "Organization (local installation)",
            "data_categories": [
                "organization_name",
                "industry",
                "contact_email",
            ...
            "data_subject_categories": ["organization owner"],
            "recipients": ["installation operator", "organization owner"],
            "lawful_basis": LegalBasis.CONTRACT,
            "retention_days": 365 + 90,  # active contract + 90-day export window
            "security_measures": "Local provisioning only, TLS in transit, password policy, audited provisioning events.",
    ...
    (
        ...
        {
            "name": "Tenant Support",
            "purpose": "Optional per-individual support access within the single installation when the organization operator explicitly configures it. No central or upstream support path exists by default.",
            "controller": "Organization (local installation)",
            "data_categories": [
                "support_user_id",
                "grant_reason",
            ...
            "data_subject_categories": ["organization owner", "support users"],
            "recipients": ["installation operator (if configured)", "organization owner"],
            "lawful_basis": LegalBasis.LEGITIMATE_INTERESTS,
            "retention_days": 365,
            "security_measures": "No central support-account path; access remains under the local server owner.",
            "cross_border_transfer": False,
        },
    ),
    (
